...
namespace InteractiveInspector
{
    public class Program
    {
        ...
        static RECT ConvertRect(UIAutomationClient.tagRECT r)
        {
            return new RECT { Left = r.left, Top = r.top, Right = r.right, Bottom = r.bottom };
        }

        static bool PointInRectScreen(int x, int y, RECT r)
        {
        ...
        static void CollectAllRects(XElement element, bool orderTopDown)
        {
            ...
            foreach (var child in element.Elements("Element"))
                CollectAllRects(child, orderTopDown);
        ...
        static void Main()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    if (qs.TryGetValue("name", out string target))
